...
def execute_command_subprocess(command_tokens_list):
    command_tokens_list = list(command_tokens_list)
    resolved_executable = shutil.which(command_tokens_list[0])
    if resolved_executable is not None:
        command_tokens_list[0] = resolved_executable

    if is_windows() and command_tokens_list[0].lower().endswith(('.bat', '.cmd')):
        command = subprocess.list2cmdline(command_tokens_list)
        rc = call(command, stdout=DEVNULL, stderr=STDOUT, shell=True)
    else:
...
def main():
    ...
    try:
        ...
        print('[!] cleanup warning: %s' % cleanup_error)
